...
public class MilkStick extends ItemFood {

	public MilkStick() {
		super(8, 4, false);
		this.setAlwaysEdible();
		this.setUnlocalizedName("milkstick");
	...
	protected void onFoodEaten(ItemStack stack, World worldIn, EntityPlayer player) {
		player.addPotionEffect(new PotionEffect(Potion.regeneration.id, 300*20, 3));
		player.addPotionEffect(new PotionEffect(Potion.heal.id, 1,10));
		player.addPotionEffect(new PotionEffect(Potion.healthBoost.id, 300*20, 3));
		player.dropItem(new ItemStack(ItemTown.ds,1), false, true);
	}
	
	@Override
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List tooltip, boolean advanced) {
		tooltip.add(i18nHelper.translate("item.milkstick.tooltip"));
	}
}
